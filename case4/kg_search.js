...
var allCells;
var found = false;
//loads the function init once the window loads
window.onload = init;
//this function inserts content into the page and sets up the event functions and listeners
function init() {
...
//This function shows what letter has been chosen
function startRecording(e) {
   document.getElementById("pickedLetters").value += e.target.textContent;
   //changes the color of the letter chosen
...
//This function continues choosing letters so long as the mouse is held down
function continueRecording(e) {
   //changes the color of letters that are hovered over while the mouse is still held down
   if (e.target.style.backgroundColor !== "rgb(28, 255, 132)") {
      e.target.style.backgroundColor = "rgb(255, 197, 153)";
   }
   document.getElementById("pickedLetters").value += e.target.textContent;
}
//This function makes it so the leeters are no longer chosen once the mouse is let go
function stopRecording() {
   //stops recording letters
   for (var i = 0; i < allCells.length; i++) {
...
//Checks if letters spell out any of the corresponding words
function checkLetters() {
   var currentLetters = document.getElementById("pickedLetters").value;
   var wordList = document.querySelectorAll("ul#wordSearchList li");
   //checks the letters that were chosen and in what order
   for (var i = 0; i < wordList.length; i++) {
   ...
   for (var i = 0; i < allCells.length; i++) {
      //changes the color of the letters depending on the words that were spelt out
      if (allCells[i].style.backgroundColor !== "rgb(28, 255, 132)") {
         //changes the letters to rgb(28, 255, 132) if the letters corresponded to one of the words
         if (allCells[i].style.backgroundColor === "rgb(255, 197, 153)" && found) {
            allCells[i].style.backgroundColor = "rgb(28, 255, 132)";
            //changes the letters back to the original color if they did not correspond to any words
         } else {
